...
def metricsScores(y_true, pred_proba, thres=0.50):
    ...
    return metrics_list
def dataRead( posPath, negPath, testPosPath=None, testNegPath=None, testfrequence=5, frequence=5, header=None,
             testHeader=None, ):
    # 训练集
...
def delete70NaN(Xtrain,columnNumber = 40,threshold = 0.7,):
    '''
    forty column
    threshol 80%
    '''
...
if __name__ == '__main__':
    # for itern in range(4,8):
        ...
        testnumber = 1
        for testnumber in range(1,3):
            predSavePath = '../otherTools/RF_pred'+str(testnumber)
            # test = 'COSMIC_test1'
            # test = 'ICGC_test2'
            mutationNumber = 6
            posPath = '../data-train2CosmicContainTCGA/out/train_pos_closeby{}.vcf'
            negPath = '../data-train2CosmicContainTCGA/out/train_neg_closeby{}.vcf'
            testNegPath = '../duplicationCS/out/test'+str(testnumber)+'_neg_closeby'+str(mutationNumber)+'_process_cs_quchong_closeby_match_features.txt'
            testPosPath = '../duplicationCS/out/test'+str(testnumber)+'_pos_closeby'+str(mutationNumber)+'_process_cs_quchong_closeby_match_features.txt'
            #mrmr选择的列名（特征选择后所需的列）
            mrmrCol = [5,18,23,25,24,31,22,21,26,33,37,35,38,32,36,17,34,20,0,19]  # 40列的列名
            # mrmrCol = [5,18,23,24,31,22,21,33,35,38,32,34,20,0,19]  # 40列的列名,去除缺失值
            #交叉验证折数
            cv = 10
            #去除缺失值的比例
            threshold = 0.7
            ...
            predictionScore.to_csv('./test'+str(testnumber)+'Score.txt', sep='\t')
            cross_meterics_score.to_csv('./validScore'+str(mutationNumber)+'.txt', sep='\t',columns =   ['Sen', 'Spe', 'Pre', 'F1', 'MCC', 'ACC', 'AUC', 'PRC', 'tn', 'fp', 'fn', 'tp', 'thres'])
